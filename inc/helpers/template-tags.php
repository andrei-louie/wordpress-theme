 ...
 * @return void
 */
function netwarriorservices_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( 'Posted on %s', 'post date', 'netwarriorservices' ),
 ...
 * @return void
 */
function netwarriorservices_posted_by() {
	$byline = sprintf(
		esc_html_x( ' by %s', 'post author', 'netwarriorservices' ),
 ...
 * @return bool|string
 */
function netwarriorservices_the_excerpt( $trim_character_count = 0 ) {
 ...
 */
function netwarriorservices_excerpt_more( $more = '' ) {

	if ( ! is_single() ) {
		$more = sprintf( '<a class="netwarriorservices-read-more text-white mt-3 btn btn-info" href="%1$s">%2$s</a>',
			get_permalink( get_the_ID() ),
			__( 'Read more', 'netwarriorservices' )
		);
	}
...
/**
 * NetWarriorServices Pagination.
 *
 * @return void
 */
function netwarriorservices_pagination() {
	...
	printf( '<nav class="netwarriorservices-pagination clearfix">%s</nav>', wp_kses( paginate_links( $args ), $allowed_tags ) );
}
